...
(float exectime) launch_wrapper(string run_id, int params[], int count = 0)
{
	...
	if (count < time_limit)
	{
		int pdf_proc = params[0];	// pdf_calc: the total number of processes
		int pdf_ppw = params[1];	// pdf_calc: the number of processes per worker
...
main()
{
	...
	// 1) pdf_calc: the number of processes per worker
