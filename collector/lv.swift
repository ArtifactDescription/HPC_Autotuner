...
(float exectime) launch_wrapper(string run_id, int params[], int count = 0)
{
	...
	if (count < time_limit)
	{
		...
		int lmp_l2s = params[7];	// Lammps: num of runs for the phase from liquid to solid
		int lmp_sld = params[8];	// Lammps: num of runs for the solid phase
...
main()
{
	...
	// 7) Lammps: num of runs for the phase from liquid to solid
	// 8) Lammps: num of runs for the solid phase
