...
(float exectime) launch_wrapper(string run_id, int params[], int count = 0)
{
	...
	if (count < time_limit)
	{
		...
		string srcDir = "%s/exp_bp4/pdf-%0.4i-%0.3i" % (workflow_root, gs_cs, gs_step);
		...
		int pplot_proc = 1;
		int pplot_ppw = 1;
		int nwork1 = pplot_proc %/ pplot_ppw;
		int timeout = 900 * float2int(2 ** count);

		string cmd1 = "python3";

		// mpiexec -n 1 python3 plot/pdfplot.py -i pdf.bp
		string args1[] = split("../../../../../adios2-coupled/gray-scott/plot/pdfplot.py -i pdf.bp -o pdfplot/fig", " ");
		...
		string envs1[] = [ "swift_chdir="+dir,
		       ...
		       "swift_numproc=%i" % pplot_proc,
		       "swift_ppw=%i" % pplot_ppw ];
...
(void v) failure(string run_id, int params[])
{
	...
	string output = "%0.4i\t%0.3i\t%s" % (params[0], params[1], "inf");
...
(float exectime) get_exectime(string run_id, int params[])
{
	...
	string cmd[] = [ turbine_output/"get_maxtime.sh", dir/"time_pdfplot.txt" ];
	sleep(1) =>
	...
	else
	{
		...
		if (exectime >= 0.0)
		{
			...
			string output = "%0.4i\t%0.3i\t%f"
				% (params[0], params[1], exectime);
			file out <dir/"time.txt"> = write(output);
		}
		else
		{
...
main()
{
	...
	// 2) pdfplot: the total number of processes
	// 3) pdfplot: the number of processes per worker
	int sample_num = 1;
	conf_samples = file_lines(input("smpl_plot.csv"));
